...
class AuditTrail:
    ...
    def __init__(self, secret_key: Optional[str] = None):
        ...
        if not resolved_key:
            ...
            # Logs a warning so operators know to set AUDIT_SECRET_KEY for persistence.
            resolved_key = secrets.token_hex(32)
            import warnings
            warnings.warn(
                "AUDIT_SECRET_KEY not set; generated an ephemeral key. "
                "Set AUDIT_SECRET_KEY environment key for audit trail persistence across restarts.",
                RuntimeWarning,
                stacklevel=2,
            )
